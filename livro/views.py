...
def ver_livros(request, id):
    if request.session.get('usuario'):
        ...
        if request.session.get('usuario') == livros.usuario.id:
            ...
            'usuario_logado': request.session.get('usuario'), 'form': form, 'id_livro':id, 'form_categoria': form_categoria})
        else:
            return
    return redirect('/auth/login/?status=2')
...
def cadastar_categoria(request):
    ...
    return HttpResponse(f"{nome} {descricao} {id_usuario}")
